...
export type ProcessStream = S.IO<CE.CommandError, Byte>
...
export function fromReadableStream(
  readable: () => NodeJSReadableStream
): S.IO<CE.CommandError, Byte> {
  return S.mapError_(
    NS.streamFromReadable(readable),
...
export function linesStream(self: ProcessStream): S.IO<CE.CommandError, string> {
  return S.aggregate_(S.aggregate_(self, utf8Decode), splitLines)
}
